...
public class Wolf_skill_tmp : MonoBehaviour
{
    ...
    IEnumerator InstantSkill()
    {
        ...
        {
            if (enemy.GetComponent<Status>().UnitSide == eUnitSide.Allied && enemy.GetComponent<Status>().hitPriority != eHitPriority.TOWER)
            {
                GameObject bullet = GameObject.Instantiate(effect, enemy.GetComponent<Status>().ProjecttileArrivalLocation.transform.position + new Vector3(0, 3.0f, 0), effect.transform.rotation);
