...
namespace Luna_Network
{
    ...
    class MessageResolver
    {
        ...
        bool read_until(byte[] buffer, ref int src_position, int offset, int transffered)
        {
            ...
            {
                // 들어온 데이터 만큼 다 읽은 상태이므로 더이상읽을 데이터가 없다.
                return false;
            }

            // 일거와야 할 바이트
            // 데이터가 분리되어 올 경우 이전에 읽어 놓은 값을 빼줘서 부족한 만큼 읽어올 수도 있도록 계산해준다.
        ...
        /// 데이터가 남아 있을 때 까지 계속 패킷을 만들어 callback 을 호출 해 준다.
