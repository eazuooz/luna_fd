...
namespace Luna_Network
{
    ...
    public class LunaPacket
    {
        ...
        public static LunaPacket create(Int16 protocol_id)
        {
            LunaPacket packet = PacketBufferManager.pop();
            packet.set_protocol(protocol_id);
            return packet;
        }
        public static void destroy(LunaPacket packet)
        {
            PacketBufferManager.push(packet);
        }

        public LunaPacket(byte[] buffer, IPeer owner)
        {
            // 참조로만 보관하여 작업한다.
        ...
        public void copy_to(LunaPacket target)
        {
