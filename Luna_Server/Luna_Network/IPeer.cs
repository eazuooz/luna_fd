...
namespace Luna_Network
{
    ...
    /// 서버일 경우 :
    ///     하나의 클라이언트 객체를 나타낸다.
    ///     이 인터페이스를 구현한 객체를 CNetworkService클래스의 session_created_callback호출시 생성하여 리턴시켜준다.
    ///     객체를 풀링할지 여부는 사용자가 원하는대로 구현한다.
    ///     
    /// 클라이언트의 경우 :
    ///     접속한 서버 객체를 나타낸다.
    ///     
    /// 이 클래스의 모든 메소드는 thread unsafe하므로 공유자원에 접근할 떄는 동기화처리가 필요하다.
    /// </summary>
    public interface IPeer
    {
        ...
        void send(LunaPacket msg);


        void disconnect();


        void process_user_operation(LunaPacket msg);
    }
}
