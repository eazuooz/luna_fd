...
namespace Luna_Network
{
    class Listener
    {
        ...
        /// 하나의 접속처리가 완료된 후 다음 accept를 수행하기 위해서 event 객체를 통해 흐름을 제어하도록 구현되어 있습니다.
        /// </summary>
        void do_listen()
        {
            ...
            while (true)
            {
                // SocketAsyncEventArgs를 재사용 하기 위해서 null로 만들어준다.
                this.accept_args.AcceptSocket = null;

                bool pending = true;
                try
                {
                    // 비동기 accept를 호출하여클라이언트의 접속을 받아들입니다.
                    // 비동기 메소드 이지만 동기적으로 수행을 완료될 경우도 있으니
                    // 리턴값을 확인하여 분기시켜주어야 합니다.
                ...
                // 즉시 완료 되면 이벤트가 발생하지 않으므로 리턴값이 false 일 경우 콜백메소드를 직접 호출해 줍니다.
                // pending상태라면 비동기 요청이 들어간 상태이므로 콜백 메소드를 기다리면 됩니다.
        ...
        void on_accept_completed(object sender, SocketAsyncEventArgs e)
        {
            if (e.SocketError == SocketError.Success)
            {
                ...
                // 외부로 넘기기 위해서 콜백 메소드를 호출해도록 합니다.
                // 이유는 소켓 처리부와 컨텐츠 구현부를 분리해주기 위함입니다.
                // 컨텐츠 구현부분은 자주 바뀔 가능성이 높지만, 소켓 Accept부분은 상대적으로 변경이 적은 부분이기 때문에
                // 양쪽을 분리시켜주는 것이 좋습니다.
                // 또한 클래스 설계방침에 따라 Listen에 관련된 코드만 존재하도록 하기 위한 이유도 있습니다.
