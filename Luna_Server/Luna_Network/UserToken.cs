...
namespace Luna_Network
{
    public class UserToken
    {
        ...
        Queue<LunaPacket> sending_queue;
        // sending-queue lock 처리에 사용 되는 객체
        ...
        public UserToken()
        {
            ...
            this.peer = null;
            this.sending_queue = new Queue<LunaPacket>();
        }
		...
		{
			LunaPacket clone = new LunaPacket();
			msg.copy_to(clone);
		...
		void start_send()
		{
			lock (this.cs_sending_queue)
			{
				...
				LunaPacket msg = this.sending_queue.Peek();
		...
		public void process_send(SocketAsyncEventArgs e)
		{
			...
			lock (this.cs_sending_queue)
			{
				...
				// 전송 완료된 패킷을 큐에서 제거한다.
				//LunaPacket packet = this.sending_queue.Dequeue();
				//LunaPacket.destroy(packet);
		...
		//void send_directly(LunaPacket msg)
		//{
		//	msg.record_size();
		//	this.send_event_args.SetBuffer(this.send_event_args.Offset, msg.position);
		//	Array.Copy(msg.buffer, 0, this.send_event_args.Buffer, this.send_event_args.Offset, msg.position);
		...
		public void start_keepalive()
		{
			...
			{
				LunaPacket msg = LunaPacket.create(0);
				msg.push(0);
				send(msg);
			}, null, 0, 3000);
		}
